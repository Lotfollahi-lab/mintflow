...
class GNNDisentangler(nn.Module):
    ...
    def forward(self, batch, prob_maskknowngenes:float, ten_xy_absolute:torch.Tensor):
        ...
        sigmaxint = torch.concat(
            [sigmaxint_raw[:,0:batch.batch_size], torch.clamp(sigmaxint_raw[:,batch.batch_size::], min=0.1, max=torch.inf)],
            1
        )
        sigmaxspl = torch.concat(
            [sigmaxspl_raw[:, 0:batch.batch_size], torch.clamp(sigmaxspl_raw[:, batch.batch_size::], min=0.1, max=torch.inf)],
            1
        )
