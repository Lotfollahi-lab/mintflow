...
parser = argparse.ArgumentParser(
    description='This script creates (or recreates) the distanglments figures (violin plots/joint plots).\n This sciprts takes in the output path when `mintflow_cli.py` was originally run.',
    formatter_class=RawTextHelpFormatter
)

parser.add_argument(
    '--path_output_mintflow_cli_dot_py',
    type=str,
    help="The output path when `mintflow_cli.py` was originally run."
)
...
# read the original args when mintflow_cli.py script was run ===
with open(os.path.join(args.path_output_mintflow_cli_dot_py, 'ConfigFilesCopiedOver', 'args.yml'), 'r') as f:
    dict_args_mintflowclidotpy = yaml.load(f, Loader=yaml.Loader)  # the path tails are saved only.
...
config_data_test = parse_config_data_test.parse(
    os.path.join(
        args.path_output_mintflow_cli_dot_py,
        'ConfigFilesCopiedOver',
        dict_args_mintflowclidotpy['file_config_data_test']
    )
)
...
config_training = parse_config_training.parse(
    os.path.join(
        args.path_output_mintflow_cli_dot_py,
        'ConfigFilesCopiedOver',
        dict_args_mintflowclidotpy['file_config_training']
    )
)
...
disentanglement_jointplot.vis(
    ...
    list_LR=list_LR,
    fname_dump_red=os.path.join(args.path_output_mintflow_cli_dot_py, 'Results_{}'.format("rownormalised" if args.flag_rownorm_adatadotX else "not_rownormalised"), 'JointPlots', 'jointplot_alltissuescombined_red.png'),
    fname_dump_blue=os.path.join(args.path_output_mintflow_cli_dot_py, 'Results_{}'.format("rownormalised" if args.flag_rownorm_adatadotX else "not_rownormalised"), 'JointPlots', 'jointplot_alltissuescombined_blue.png'),
