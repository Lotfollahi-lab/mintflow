...
parser.add_argument(
    ...
    type=str,
    help='The yaml file to configure how MintFlow is expected to read training data.\n' +\
...
parser.add_argument(
    ...
    type=str,
    help='The yaml file to configure how MintFlow is expected to read testing data.\n' +\
    'Please refere to TODO: for an example of such file.'
)
...
parser.add_argument(
    '--file_config_model',
    type=str,
    help='The yaml file to configure architecture of MintFlow modules (e.g. encoders etc.) .\n' +\
...
parser.add_argument(
    ...
    type=str,
    help="The yaml file to configure MintFlow's training (e.g. learning rate, number of epochs, etc.) .\n" +\
    'Please refere to TODO: for an example such file.'
)
...
parser.add_argument(
    '--path_output',
    type=str,
    help="The output path where MintFlow will dump the output (some figures, embeddings, etc.)"
)
...
dict_temp = {
    'headxint':mintflow.modules.gnn_disentangler.ModeArch.HEADXINT,
    'headxspl':mintflow.modules.gnn_disentangler.ModeArch.HEADXSPL,
    'headboth':mintflow.modules.gnn_disentangler.ModeArch.HEADBOTH,
    'twosep':mintflow.modules.gnn_disentangler.ModeArch.TWOSEP,
}
...
module_vardist = BASE_PATH.vardist.MintFlowVarDist(
    module_genmodel=BASE_PATH.generativemodel.MintFlowGenerativeModel(
        **kwargs_genmodel
    ),
...
if args.flag_verbose:
    print("MintFlow vardist module was created on {}.".format(device))
...
# log the mintflow module
with open(os.path.join(args.path_output, "log_mintflow_module.txt"), 'w') as f:
...
# dump the MintFlow model as well as the inferred latent factors ===
...
torch.save(
    {
        'module_mintflow': module_vardist,
     },
    os.path.join(
...
if config_training['flag_finaleval_createanndata_alltissuescombined']:
    ...
    # create the normalised version: adata_mintflowOutput_norm
    list_anndata_norm = []
    for idx_sl, sl in enumerate(test_list_slice.list_slice):
        ...
        for k in dict_temp.keys():
            sl.adata.obsm['mintflow_{}'.format(k)] = dict_temp[k] + 0.0
    ...
    adata_norm.uns['mintflow_map_CT_to_mintflowCT'] = test_list_slice.map_CT_to_mintflowCT
    adata_norm.uns['mintflow_map_Batchname_to_mintflowBatchID'] = test_list_slice.map_Batchname_to_mintflowBatchID
    ...
    # create the unnormalised version: adata_mintflowOutput_unnorm
    list_anndata_unnorm = []
    for idx_sl, sl in enumerate(test_list_slice.list_slice):
        dict_temp = torch.load(
            os.path.join(path_dump_checkpoint, 'predictions_slice_{}.pt'.format(idx_sl + 1))
        ...
        for k in dict_temp.keys():
            sl.adata_before_scppnormalize_total.obsm['mintflow_{}'.format(k)] = dict_temp[k] + 0.0
    ...
    adata_unnorm.uns['mintflow_map_CT_to_mintflowCT'] = test_list_slice.map_CT_to_mintflowCT
    adata_unnorm.uns['mintflow_map_Batchname_to_mintflowBatchID'] = test_list_slice.map_Batchname_to_mintflowBatchID
